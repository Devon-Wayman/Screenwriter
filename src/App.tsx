...
import { downloadScreenplayPdf, layoutScreenplay, type PdfOptions } from './pdf';
...
export default function App() {
  ...
  const pdfLayout = useMemo(() => layoutScreenplay(parsed, pdfOptions), [parsed, pdfOptions]);
  const canUndo = useMemo(() => history.current.canUndo, [historyRevision]);
  const canRedo = useMemo(() => history.current.canRedo, [historyRevision]);
  ...
  function restoreHistoryEntry(entry: HistoryEntry | null, action: 'Undo' | 'Redo') {
    ...
    setContent(entry.text); setDirty(entry.text !== savedContent.current); setStatus(action); setHistoryRevision((value) => value + 1);
  ...
  function recordContent(nextText: string, selectionStart: number, selectionEnd: number, group = 'structural') {
    ...
    setContent(nextText); setDirty(nextText !== savedContent.current); setHistoryRevision((value) => value + 1);
  }
  ...
  function resetHistory(nextText: string, selectionStart = 0) {
    ...
    savedContent.current = nextText; setHistoryRevision((value) => value + 1);
  }
  ...
  return <div className={`app-shell ${focusMode ? 'focus-mode' : ''}`}>
    ...
    <main className={`workspace ${sidebarOpen ? '' : 'files-closed'}`}>
      ...
      <section className="editor-panel">
        ...
        <div className="editor-stack">
          <pre ref={highlightLayer} className="highlight-layer" style={{ fontSize }} aria-hidden="true">{parsed.lines.map((line) => <span key={`${line.start}-${line.index}`} style={{ color: syntaxColors[line.type] }}>{line.text}{line.index < parsed.lines.length - 1 ? '\n' : ''}</span>)}{' '}</pre>
    ...
    {pdfOpen && <div className="modal-backdrop" role="presentation" onMouseDown={(event) => { if (event.target === event.currentTarget) setPdfOpen(false); }}><section className="pdf-dialog" role="dialog" aria-modal="true" aria-labelledby="pdf-dialog-title">
      ...
      <div className="pdf-dialog-body"><aside>
        ...
        {page.blocks.map((block, blockIndex) => <div className={`preview-block ${block.type}`} key={blockIndex} style={{ left: `${block.x / pdfLayout.width * 100}%`, top: `${block.y / pdfLayout.height * 100}%`, width: `${block.width / pdfLayout.width * 100}%`, textAlign: block.align }}>

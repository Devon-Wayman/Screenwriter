  ...
  return <div className={`app-shell ${focusMode ? 'focus-mode' : ''} ${focusMode && focusPdfEnabled ? 'focus-live-pdf' : ''}`}>
    <header className="topbar">
      ...
      <nav className="actions">
        ...
        {!standalone && <><button onClick={undo} disabled={!canUndo} title="Undo (Ctrl/⌘+Z)">Undo</button><button onClick={redo} disabled={!canRedo} title="Redo (Ctrl+Y or Ctrl/⌘+Shift+Z)">Redo</button></>}<button onClick={() => setFindOpen(!findOpen)}>Find</button>

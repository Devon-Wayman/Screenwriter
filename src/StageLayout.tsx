...
interface Props { documentName: string; sceneLines: FountainLine[]; onStatus: (message: string) => void; standalone?: boolean }
...
export default function StageLayout({ documentName, sceneLines, onStatus, standalone = false }: Props) {
  ...
  useEffect(() => {
    ...
    const timer = window.setTimeout(() => {
      ...
      fetch(`/api/stage-layouts/${encodeURIComponent(documentName)}`, { method: 'PUT', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(document) })
        .then(async (response) => { if (!response.ok) throw new Error((await response.json().catch(() => ({}))).error || `Stage layout save failed (${response.status}).`); setLayoutError(''); setSaveState('saved'); onStatus(standalone ? 'Stage layout saved locally' : 'Stage layout saved on NAS'); })
  ...
  }, [document, loaded, documentName, standalone]);
  ...
  return <main className="stage-layout-workspace">
    ...
    <section className="stage-designer"><header><div><small>ACTIVE SCENE</small><h2>{activeScene?.heading}</h2><span className={`stage-save-state ${saveState}`}>{saveState === 'loading' ? 'Loading layout…' : saveState === 'pending' ? 'Changes waiting to save…' : saveState === 'saving' ? 'Saving layout…' : saveState === 'error' ? `Layout error: ${layoutError}` : standalone ? 'Layout saved locally' : 'Layout saved on NAS'}</span></div><div className="shape-tools">{(['rectangle','circle','line','label','light','actor'] as StageShapeType[]).map((type) => <button key={type} onClick={() => addShape(type)}>+ {shapeDefaults[type].label}</button>)}</div></header>
